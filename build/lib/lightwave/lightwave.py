...
def _send_message(self, msg):
    ...
    the_queue.put_nowait(msg)
    if thread is None or not self.thread.isAlive():
        thread = threading.Thread(target=self._send_queue)
        thread.start()
...
def _send_queue(self):
    ...
    while not the_queue.empty():
        self._send_reliable_message(the_queue.get_nowait())
